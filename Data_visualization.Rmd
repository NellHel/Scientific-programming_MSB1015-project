...
```{r HHI}
plot(rownames(HHI), HHI, type = "h", xlab = "Year", ylab = "HHI index", main = "HHI index over the years")
```
...
```{r km}
plot(km_avg$Year, km_avg$KM_average, type = "h", xlab = "Year", ylab = "Kilometer", main = "Average length of stage over the years")
plot(km_avg$Year, km_avg$n, type = "h",  xlab = "Year", ylab= "Number of stages", main = "Average number of stages over the years")
```

Number of teams per year

```{r teams_no}
plot(teams_number$Year, teams_number$teamnumber, type = "h", xlab = "Year", ylab = "Number of teams", main = "Number of teams over the years")
```

Doping cases per year

```{r doping}
plot(doping_no$Year, doping_no$Doping_Cases, type = "h", xlab = "Year", ylab = "Number of doping cases", main = "Doping cases over the years")
...
```{r budget}
plot(budget$Year, budget$`WorldTour average budget per team`, type = "l", xlab = "Year", ylab = "Inflation adjusted team budget (Euro)", main = "Average team budgets")
```
...
```{r plot_win_gc_share}
ggplot(PCoA1_2, aes(PCoA1, PCoA2, color = Win_Share, size = GC_Share)) +
  geom_point() +
  theme_minimal() +
  labs(title = "PCoA colored by its features (win and gc share)",
       x = paste0("PCoA1 (", round(pcoa_res$values$Relative_eig[1]*100, 1), "%)"),
       y = paste0("PCoA2 (", round(pcoa_res$values$Relative_eig[2]*100, 1), "%)"),
       color = "Win Share",
       size = "GC Share"
...
```{r plot_budget}
ggplot(PCoA1_2, aes(PCoA1, PCoA2, color = Budget)) +
  geom_point() +
  theme_minimal() +
